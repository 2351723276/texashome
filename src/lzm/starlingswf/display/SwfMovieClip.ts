...
//                        textfield["bold"] = data[18];
        ...
        public addEventListener1(type:string, listener:Function, thisObject:any, useCapture:boolean = false, priority:number = 0):void{
        ...
        public removeEventListener1(type:string, listener:Function,thisObject:any,useCapture:boolean = false):void{
            this.removeEventListener(type,listener,thisObject,useCapture);
            this._hasCompleteListener = this.hasEventListener(egret.Event.COMPLETE);
        }

        private addChildNew(child:egret.DisplayObject){
        ...
        public getDisplay(name:string):egret.DisplayObject{
            for(let k in this._displayObjects){
                let arr:any[] = this._displayObjects[k];
                let len = arr.length;
                for(let i:number = 0; i < len ; i ++){
                    if(arr[i].name == name){
                        return arr[i];
